"""
exp16_spikujici_rozhrani.py — učit dvanáct synapsí místo šedesáti tisíc neuronů.

PREREGISTRACE (zapsáno před během, 14. 9. 2026):
Exp13 až exp15 ukázaly, proč se dosud nic nenaučilo: učené rozhraní
(12 synapsí z graduované vrstvy na DNa02) nese jen **1,1 % a 0,6 %** váhy
vstupu do toho neuronu. Zbylých 99 % přichází ze spikující části mozku
(365 a 381 synapsí). Náhodné přenastavení těch dvanácti vah nezmění
chování o víc než 6 p. b., ať je úloha jakákoli.

Tenhle pokus učí tu druhou cestu: **746 synapsí ze spikující části na
DNa02**. Že tam vliv je, je změřené — náhodné přenastavení posunulo
výsledek v rozsahu 0,338 až 0,748 proti výchozím 0,674.

Úloha: držet cíl 30° stranou, ne ve středu (spojitá míra, bez podlahy
i stropu; výchozí 0,674). Pravidlo: node perturbation, přičítací update,
odměna jako chyba predikce. Měřidlo: pevná zkušební sada.

  R1: měřidlo drží, sham se nehne o víc než 2 p. b.
  R2: medián paired − placebo > 5 p. b.
...
def blok(a, h, delka, rezim, rng, sila, sigma, baseline):
    n = len(a.i_spike)
    stopy_spike = {"vlevo": np.zeros(n, np.float32), "vpravo": np.zeros(n, np.float32)}
    ok = 0.0
    ...
    for _ in range(delka):
        ...
        for strana in ("vlevo", "vpravo"):
            stopy_spike[strana] = ROZPAD * stopy_spike[strana] + a._posledni_sum[strana] * spn
        h.krok(k.zataceni)
        ok += miraKrok(h)
    r = 2.0 * (ok / delka) - 1.0
    if rezim != "sham":
        delta = float(rng.normal(0, 0.3)) if rezim == "placebo" else r - baseline["v"]
        a.odmena_perturbaci(stopy_spike, delta, sila=sila)
...
def main():
    ...
    if not r1:
        ...
        verdikt = "Učí se — přes spikující rozhraní to jde"
    else:
        verdikt = "ZÁPORNÝ: neučí se ani přes 746 synapsí, které nesou 99 % vstupu"
    log.info("VERDIKT: %s", verdikt)
    (ROOT / "outputs" / "exp16.json").write_text(
